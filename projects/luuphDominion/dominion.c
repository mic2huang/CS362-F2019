...
int playBaron(struct gameState *state, int choice1)
{
    ...
    // if the user chooses to discard an Estate
    if (choice1 > 0)
    {
        ...
        {
            // if an Estate card is found on the current player's hand
            if (state->hand[currentPlayer][p] == estate)
            {
                // add 4 coins to the amount of coins
                state->coins += 4;

                // remember the card to be discarded
                ...
                for (; p < state->handCount[currentPlayer]; p++)
                {
                    ...
                    /* ASSIGNMENT 2 - 4 - BARON - BUG 2 - OVERWRITE hand */
                    state->hand[currentPlayer][p + 1] = state->hand[currentPlayer][p];
                }
            ...
            {
                // if there was no Estate card, the current player must gain one
                if (DEBUG)
                {
                ...
                if (supplyCount(estate, state) > 0)
                {
                    ...
                    // take an Estate out of the supply
                ...
                card_not_discarded = 0; // exit the loop */ (REMOVED in Assignment 5 - huynthu)
            }
            else
            {
                p++; // next card
    ...
    {
        // if the player chooses not to discard and Estate, they must gain one
        if (supplyCount(estate, state) > 0)
        {
            gainCard(estate, state, 0, currentPlayer);

            // take an Estate out of the supply
            /* ASSIGNMENT 2 - 4 - BARON - BUG 2 - FORGETS TO REDUCE SUPPLY */
            /* state->supplyCount[estate]--; */
            if (supplyCount(estate, state) == 0)
            {
                isGameOver(state);
...
int playMinion(struct gameState *state, int handPos, int choice1, int choice2)
{
    ...
    /* ASSIGNMENT 2 - 4 - MINION - BUG 1 - ACTION NOT UPDATED */
    /* state->numActions++;  // +1 action */

    // discard card from hand
    discardCard(handPos, currentPlayer, state, 0);

    // if the user wants to use this card to gain 2 coins, then do it
    if (choice1)
    {
    ...
    {
        // otherwise, discard hand, redraw 4, and then other players with
        ...
        for (int i = 0; i < state->numPlayers; i++)
        {
            if (i != currentPlayer && state->handCount[i] > 4)
            {
                ...
                /* ASISGNMENT 2 - 4 - MINION - BUG 2 - NOT DRAW 4 */
                /* // draw 4 */
                /* for (int j = 0; j < 4; j++) { */
                /*     drawCard(i, state); */
                /* } */
...
int playAmbassador(struct gameState *state, int handPos, int choice1, int choice2)
{
    ...
    /* ASSIGNMENT 2 - 4 - AMBASSADOR - BUG 2 - TRASHED CARD NOT DISCARDED */
...
int playTribute(struct gameState *state, int *tributeRevealedCards)
{
    int currentPlayer = whoseTurn(state);

    /* ASSIGNMENT 2 - 4 - TRIBUTE - BUG 1 - OVERFLOW ARRAY */
    int nextPlayer = (currentPlayer + 1); // % state->numPlayers;
    /* int nextPlayer = (currentPlayer + 1) % state->numPlayers; */

    // if (0 discard, 0 deck) or (0 discard, 1 deck) or (1 discard, 0 deck)
    if (state->discardCount[nextPlayer] + state->deckCount[nextPlayer] <= 1)
    {
        // if the next player still has some card on their hand
        if (state->deckCount[nextPlayer] > 0)
        {
            // mark the top card on the hand to be revealed
            tributeRevealedCards[0] =
                state->deck[nextPlayer][state->deckCount[nextPlayer] - 1];

            // and then discard this card from their hand
            state->deckCount[nextPlayer]--;
        }
        else if (state->discardCount[nextPlayer] > 0)
        {
            // if the next player has some discarded card, tribute the top card
            // on their hand
            tributeRevealedCards[0] =
    ...
    {
        // if the next player doesn't have any card on their hand
        if (state->deckCount[nextPlayer] == 0)
        {
            ...
            /* ASSIGNMENT 2 - 4 - TRIBUTE - BUG 2 - FORGET TO SHUFFLE */
            /* shuffle(nextPlayer,state);  // shuffle the deck */
        }

        // reveal the top 2 cards on their hand
        tributeRevealedCards[0] =
            state->deck[nextPlayer][state->deckCount[nextPlayer] - 1];
        state->deck[nextPlayer][state->deckCount[nextPlayer]--] = -1;
        state->deckCount[nextPlayer]--;
...
int playMine(struct gameState *state, int handPos, int choice1, int choice2)
{
    ...
    /* ASSIGNMENT 2 - 4 - MINE - BUG 1 - NOT CHECK FOR TREASURE */
    ...
    /* ASSIGNMENT 2 - 4 - MINE - BUG 2 - FORGET TO GAIN THE TREASURE */
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case tribute:
        return playTribute(state, tributeRevealedCards);
